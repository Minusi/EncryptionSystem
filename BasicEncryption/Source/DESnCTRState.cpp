...
DESnCTRState::DESnCTRState() : Key(CryptoPP::SecByteBlock(CryptoPP::DES::DEFAULT_KEYLENGTH))
{
	//초기화 프로세스
	{
...
void DESnCTRState::ExecuteWithData()
{
	std::cout << "ENCRYPTOR:: DES-CTR 방식의 암호화 프로세스 진행중..." << std::endl;

	// 암호화 프로세스
	try
	{

		CryptoPP::CTR_Mode<CryptoPP::DES>::Encryption CBCEncryption;
	...
	try
	{
		CryptoPP::CTR_Mode<CryptoPP::DES>::Decryption CBCDecryption;
...
void DESnCTRState::ExecuteWithFile()
{
	std::cout << "ENCRYPTOR:: DES-CTR 방식의 암호화 프로세스 진행중..." << std::endl;
	std::cout << "ENCRYPTOR:: 프로세스의 타겟 확인 >> 파일 " << std::endl;

	std::string InputFilePath = DefaultTargetFile;
	std::string CipherFilePath = DefaultTargetPath + AppendDES + AppendCTR + AppendNo + CipherFileName;
	std::string DecryptedFilePath = DefaultTargetPath + AppendDES + AppendCTR + AppendNo + DecryptedFileName;
