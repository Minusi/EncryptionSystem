...
DESnCBCState::DESnCBCState() : Key(CryptoPP::SecByteBlock(CryptoPP::DES::DEFAULT_KEYLENGTH))
{
	// 초기화 프로세스
	{
...
void DESnCBCState::ExecuteWithData()
{
	std::cout << "ENCRYPTOR:: DES-CBC 방식의 암호화 프로세스 진행중..." << std::endl;
	
	// 암호화 프로세스
	try
	{

		CryptoPP::CBC_Mode<CryptoPP::DES>::Encryption CBCEncryption;
...
void DESnCBCState::ExecuteWithFile()
{
	std::cout << "ENCRYPTOR:: DES-CBC 방식의 암호화 프로세스 진행중..." << std::endl;
	std::cout << "ENCRYPTOR:: 프로세스의 타겟 확인 >> 파일 " << std::endl;

	std::string InputFilePath = DefaultTargetFile;
	std::string CipherFilePath = DefaultTargetPath + AppendDES + AppendCBC + AppendNo + CipherFileName;
	std::string DecryptedFilePath = DefaultTargetPath + AppendDES + AppendCBC + AppendNo + DecryptedFileName;
