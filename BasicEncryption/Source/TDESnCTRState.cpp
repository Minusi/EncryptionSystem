...
TDESnCTRState::TDESnCTRState() : Key(CryptoPP::SecByteBlock(CryptoPP::DES_EDE3::DEFAULT_KEYLENGTH))
{
	//초기화 프로세스
	{
...
void TDESnCTRState::ExecuteWithData()
{
	std::cout << "ENCRYPTOR:: 3DES-CTR 방식의 암호화 프로세스 진행중..." << std::endl;
	...
	try
	{
		...
		CryptoPP::CTR_Mode<CryptoPP::DES_EDE3>::Encryption CBCEncryption;
	...
	try
	{
		CryptoPP::CTR_Mode<CryptoPP::DES_EDE3>::Decryption CBCDecryption;
...
void TDESnCTRState::ExecuteWithFile()
{
	std::cout << "ENCRYPTOR:: 3DES-CTR 방식의 암호화 프로세스 진행중..." << std::endl;
	std::cout << "ENCRYPTOR:: 프로세스의 타겟 확인 >> 파일 " << std::endl;

	std::string InputFilePath = DefaultTargetFile;
	std::string CipherFilePath = DefaultTargetPath + AppendTDES + AppendCTR + AppendNo + CipherFileName;
	std::string DecryptedFilePath = DefaultTargetPath + AppendTDES + AppendCTR + AppendNo + DecryptedFileName;
